...
def prepare_cross_domains_model_specific():
    ...
    for model_patterns in model_sets:
        ...
        for split in ['train','valid','test']:
            text_path = f"{data_dir}/{split}.csv" 
            rows = list(csv.reader(open(text_path,encoding='utf-8-sig')))[1:]
...
def prepare_unseen_models():
    ...
    for model_patterns in model_sets:
        ...
        for split in ['train','valid','test', 'test_ood']:
            text_path = f"{data_dir}/{split}.csv" if split != 'test_ood' else f"{data_dir}/test.csv" 
            rows = list(csv.reader(open(text_path,encoding='utf-8-sig')))[1:]
        ...
        out_split_samples['test_ood'] = test_pos_rows[:len_aug] + out_split_samples['test_ood']
