...
1. source.zip: source texts of human-written texts and corresponding texts generated by LLMs, with sources specified as csv file names. For instance, the file "cmv_machine_specified_gpt-3.5-trubo.csv" includes texts from the CMV domain that were generated by the "gpt-3.5-trubo" model using specific prompts. On the other hand, "cmv_human" contains human-written CMV texts.
2. processed.zip: a refined version of ''source'' where texts of poor quality are filtered out. We recommend experimenting with this version for testifying your detection methods.
3. testbeds_processed.zip: 6 testbeds based on the ''processed'' version.
4. wilder_testsets.zip: 2 wilder test sets with texts processed.
<!-- # 🤝 Contributing -->
